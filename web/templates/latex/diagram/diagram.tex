...
\DeclareStringOption[5]{lineWidth}
...
\newcommand*{\gluon}[3][]{
    ...
    \FPeval{\nLoops}{round((\length / \gluonWidth / 2) - 1, 0)}

    % if they want the end caps (to reduce nLoops by one)
    \ifstyle@endcaps
    ...
    % we fit at least one loop between the two points
        ...
        \ifstyle@endcaps
            ...
            \draw (0,0) .. controls (\halfWidth, 0) and (0, \ymin) .. (\halfWidth, \ymin);
            ...
            \foreach \loopNumber in {1,...,\nLoops}{
                ...
                \draw (\x, \ymin) .. controls (\xFinal, \ymin) and (\xFinal, \ymax) .. (\xMid, \ymax);
                % draw the second half of the loop
                \draw (\xMid, \ymax) .. controls (\x, \ymax) and (\x, \ymin) .. (\xFinal, \ymin);
            }
            ...
            \draw (\finalStart, \ymin) .. controls (\finalStop, \ymin) and (\finalStart, 0) .. (\finalStop, 0);
        ...
        \else
            ...
            \foreach \loopNumber in {0,...,\nLoops}{
                ...
                % draw the first half of the loop
                \draw (\x,0) .. controls (\xFinal, 0) and  (\xFinal, \height) .. (\xMid, \height);
                % draw the second half of the loop
                \draw (\xMid,\height) .. controls (\x, \height) and  (\x, 0) .. (\xFinal, 0);
            }
        \fi
    }
}
